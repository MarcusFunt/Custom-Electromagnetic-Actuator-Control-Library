...
| `fem/quality.py` (`emac-femqc`) | Physical-sanity QC for force LUTs: flags tables that violate the coupling's invariants (finite, nulls, far-field decay, restoring sign, odd symmetry, current linearity, monotone tail). Triage a finished run's `.npz` tables; exits non-zero if any are suspect. |
| `fem/convergence.py` (`emac-femcheck`) | Pre-flight a run: mesh-convergence check for a coil (is the force settled? what's the coarsest safe mesh?) plus a wall-clock estimate for the full sweep. Requires FEMM. |
